...
if (($manifest['name'] ?? '') !== 'form-builder' || ($manifest['version'] ?? '') !== '1.7.9') throw new RuntimeException('Unexpected release manifest.');
if (!class_exists('ZipArchive')) throw new RuntimeException('PHP zip extension is required to build the package.');
$output = $argv[1] ?? (dirname($root) . '/form-builder-1.7.9.zip');
